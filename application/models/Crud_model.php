...
class Crud_model extends CI_Model
{
	...
	public function get_exam_categories($param1 = "")
	{
		if ($param1 != "") {
			$this->db->where('id', $param1);
		}
	...
	public function get_exam_category_details_by_id($id)
	{
		return $this->db->get_where('exam_category', array('id' => $id));
	}
	...
	public function add_exam_categories()
	{
		...
		$this->db->where('name', $data['name']);
		$this->db->or_where('slug', $data['slug']);
		$previous_data = $this->db->get('exam_category')->num_rows();

		if ($previous_data == 0) {
			$data['date_added'] = strtotime(date('D, d-M-Y'));
			$this->db->insert('exam_category', $data);
			return true;
		}
	...
	public function edit_exam_categories($param1)
	{
		$data['name']   = html_escape($this->input->post('name'));
		$data['slug']   = slugify(html_escape($this->input->post('name')));
		...
		$this->db->where('name', $data['name']);
		$this->db->or_where('slug', $data['slug']);
		$previous_data = $this->db->get('exam_category')->result_array();
		...
		if ($checker) {
			$data['last_modified'] = strtotime(date('D, d-M-Y'));
			$this->db->where('id', $param1);
			$this->db->update('exam_category', $data);
	...
	public function delete_exam_categories($category_id)
	{
		$this->db->where('id', $category_id);
		$this->db->delete('exam_category');
	}
	...
	public function update_system_settings()
	{
		...
		$data['value'] = html_escape($this->input->post('slogan_top'));
		$this->db->where('key', 'slogan_top');
		$this->db->update('settings', $data);
	...
	public function delete_lesson($lesson_id)
	{
		...
		$this->db->where('quiz_id', $lesson_id);
	...
	public function update_frontend_settings()
	{
		...
		$data['value'] = $this->input->post('thongtin_chuyenkhoan');
		$this->db->where('key', 'thongtin_chuyenkhoan');
		$this->db->update('frontend_settings', $data);
	}
	...
	// updating exam
	public function edit_exam($lesson_id = "")
	{
		$data['title'] = html_escape($this->input->post('title'));
		$data['category_id'] = html_escape($this->input->post('category_id'));
		$data['duration'] = $this->input->post('duration');
	...
	// Get list exams
	public function get_list_exam()
	{
		$this->db->where('lesson_type', 'exam');
		return $this->db->get('lesson');
	}
	...
	public function get_banner_details_by_code($code)
	{
		$this->db->where('code', $code);
		return $this->db->get('banners');
	}

	public function add_banner($post_image)
	{
		if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($post_image) && !empty($post_image)) {
			$data['code'] = $this->input->post('code');
			$data['discount_percentage'] = $this->input->post('discount_percentage');
			$data['expiry_date'] = $post_image;
			$data['created_at'] = strtotime(date('D, d-M-Y'));
	...
	public function edit_banner($banner_id, $post_image)
	{
		if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($post_image) && !empty($post_image)) {
			$data['code'] = $this->input->post('code');
			$data['discount_percentage'] = $this->input->post('discount_percentage');
	...
	/** tieuchi FUNCTIONS */
	public function get_tieuchis($id = null)
	{
		if ($id > 0) {
			$this->db->where('id', $id);
		}
		return $this->db->get('tieuchis');
	}

	public function get_tieuchi_details_by_code($code)
	{
		$this->db->where('code', $code);
		return $this->db->get('tieuchis');
	}

	public function add_tieuchi($post_image)
	{
		if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($post_image) && !empty($post_image)) {
			$data['code'] = $this->input->post('code');
			$data['discount_percentage'] = $this->input->post('discount_percentage');
	...
	public function edit_tieuchi($tieuchi_id, $post_image)
	{
		if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($post_image) && !empty($post_image)) {
			$data['code'] = $this->input->post('code');
			$data['discount_percentage'] = $this->input->post('discount_percentage');
			...
			$this->db->where('id', $tieuchi_id);
			$this->db->update('tieuchis', $data);
			return true;
		} else {
			return false;
		}
	...
	public function delete_tieuchi($tieuchi_id)
	{
		$this->db->where('id', $tieuchi_id);
		$this->db->delete('tieuchis');
		return true;
	}

	/** cat_new FUNCTIONS */
	public function get_cat_new($id = null)
	{
		if ($id > 0) {
			$this->db->where('id', $id);
		}
		return $this->db->get('cat_new');
	}

	public function get_cat_new_details_by_code($code)
	{
		$this->db->where('code', $code);
		return $this->db->get('cat_new');
	}

	public function add_cat_new()
	{
		if (isset($_POST['name']) && !empty($_POST['name'])) {
			$data['name'] = $this->input->post('name');
			$data['created_at'] = strtotime(date('D, d-M-Y'));

			$this->db->insert('cat_new', $data);
			return true;
		} else {
			return false;
		}
	...
	public function edit_cat_new($cat_new_id)
	{
		if (isset($_POST['name']) && !empty($_POST['name'])) {
			$data['name'] = $this->input->post('name');
			$data['created_at'] = strtotime(date('D, d-M-Y'));

			$this->db->where('id', $cat_new_id);
			$this->db->update('cat_new', $data);
			return true;
		} else {
			return false;
		}
	...
	public function delete_cat_new($cat_new_id)
	{
		$this->db->where('id', $cat_new_id);
		$this->db->delete('cat_new');
		return true;
	}
	...
	public function get_news_details_by_code($code)
	{
		$this->db->where('code', $code);
		return $this->db->get('news');
	}

	public function add_news($post_image)
	{
		if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['desc']) && !empty($_POST['desc']) && isset($post_image) && !empty($post_image)) {
			$data['name'] = $this->input->post('name');
			$data['desc'] = $this->input->post('desc');
			$data['content'] = $this->input->post('content');
			$data['cat'] = $this->input->post('cat');
			$data['thumb'] = $post_image;
			$data['created_at'] = strtotime(date('D, d-M-Y'));
	...
	public function edit_news($news_id, $post_image)
	{
		if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['desc']) && !empty($_POST['desc']) && isset($post_image) && !empty($post_image)) {
			$data['name'] = $this->input->post('name');
			$data['desc'] = $this->input->post('desc');
	...
	public function get_menu($id = null)
	{
		...
		return $this->db->order_by('sort', 'ASC')->get('menu');
	}

	public function get_menu_sub($id = null)
	{
		$this->db->where('sub', $id);
		return $this->db->order_by('sort', 'ASC')->get('menu');
	}

	public function get_menu_details_by_code($code)
	{
		$this->db->where('code', $code);
	...
	public function add_menu()
	{
		if (isset($_POST['name']) && !empty($_POST['name'])) {
			...
			$availability = $this->db->get_where('menu', ['name' => $data['name']])->num_rows();
			if ($availability) {
				return false;
			} else {
	...
	public function edit_menu($menu_id)
	{
		if (isset($_POST['name']) && !empty($_POST['name'])) {
			...
			$availability = $this->db->get('menu')->num_rows();
			if ($availability) {
				return false;
			} else {
				$this->db->where('id', $menu_id);
	...
	{
		return $this->db->order_by('id', 'DESC')->get('news');
	}

	public function get_new_news_home()
	{
		return $this->db->limit(6)->order_by('id', 'DESC')->get('news');
	}

	public function get_dangkyhoc()
	{
		return $this->db->order_by('id', 'DESC')->get('dangkyhoc');
	}

	public function dang_ky_ngay()
	{
		if (isset($_POST['hoten']) && !empty($_POST['hoten'])) {
			$data['hoten'] = $this->input->post('hoten');
			$data['khoahoc'] = $this->input->post('khoahoc');
			$data['phone'] = $this->input->post('phone');
			$data['email'] = $this->input->post('email');
			$data['status'] = 0;
			$data['created_at'] = time();
			$this->db->insert('dangkyhoc', $data);
			return true;
		} else {
			return false;
		}
	...
	// Code by MV
	public function get_tab_course_for_you()
	{
	...
	public function get_courses_latest()
	{
		$this->db->order_by('date_added', 'DESC');
		$this->db->limit(4);
		return $this->db->get('course')->result_array();
	...
	public function getCategoryForeginOfCourse($courseId)
	{
		...
		$course = $courses[0];
		$categoryLevel1 = $this->db
			->where('id', $course['category_id'])
			->get('category')->result_array();

		$categoryLevel2 = $this->db
			->where('id', $course['sub_category_id'])
			->get('category')->result_array();

		return [
			'level1' => $categoryLevel1[0] ?? [],
			'level2' => $categoryLevel2[0] ?? [],
		];
	}

	public function get_realted_courses($id, $cat_id, $sub_cat_id)
	{
		$this->db->limit(5);
		$realted_courses = $this->get_courses($cat_id, $sub_cat_id)->result_array();
		return array_filter($realted_courses, function ($course) use ($id) {
			return $course['id'] != $id && $course['status'] == 'active';
		});
	}

	public function get_enrol_of_teacher($instructor_id)
	{
		$course_ids = $this->get_instructor_wise_courses($instructor_id, 'simple_array');
		$this->db->select('user_id');
		$this->db->distinct();
	...
	public function getOneVideoDemoCourse($course)
	{
		if (gettype($course['video_url']) === 'string' && !empty($course['video_url'])) {
			return $course['video_url'];
		}

		$lesson = $this->db->get_where('lesson', [
			'course_id' => $course['id']
		], 1)->result_array();

		if (count($lesson) === 0 || empty($lesson[0]['video_url']))
			return '';
		return $lesson[0]['video_url'];
	}
}
