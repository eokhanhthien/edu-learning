...
class Admin extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->init();
		$this->loadService('expert');
		$this->loadService('course-expert');
	}
	...
	public function exam_categories($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		if ($param1 == 'add') {
			...
			$response = $this->crud_model->add_exam_categories();
			if ($response) {
				$this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
			} else {
				$this->session->set_flashdata('error_message', get_phrase('category_name_already_exists'));
			}
			redirect(site_url('admin/exam_categories'), 'refresh');
		} elseif ($param1 == "edit") {

			$response = $this->crud_model->edit_exam_categories($param2);
			if ($response) {
				$this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
			} else {
				$this->session->set_flashdata('error_message', get_phrase('category_name_already_exists'));
			...
			$this->crud_model->delete_exam_categories($param2);
			$this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
			redirect(site_url('admin/exam_categories'), 'refresh');
		}
		$page_data['page_name'] = 'exam_categories';
		$page_data['page_title'] = 'Danh mục loại đề thi';
		$page_data['categories'] = $this->crud_model->get_exam_categories($param2);
		$this->load->view('backend/index', $page_data);
	}

	public function exams($param1 = "", $param2 = ""){
		...
		if ($param1 == 'add') {
			...
			$response = $this->crud_model->add_exam();
			if ($response) {
				$this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
			} else {
				$this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
			}
			redirect(site_url('admin/exams'), 'refresh');
		} elseif ($param1 == "edit") {

			$response = $this->crud_model->edit_exam($param2);
			if ($response) {
				$this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
			} else {
				$this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
		...
		$page_data['page_title'] = 'Danh sách đề thi';
		$page_data['exams'] = $this->crud_model->get_list_exam();
		$page_data['categories'] = $this->crud_model->get_exam_categories()->result_array();
		$this->load->view('backend/index', $page_data);	
	}
	...
	public function exam_category_form($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		if ($param1 == "add_category") {
			...
			$page_data['page_name'] = 'exam_category_add';
			$page_data['page_title'] = get_phrase('add_category');
		}
		if ($param1 == "edit_category") {

			$page_data['page_name'] = 'exam_category_edit';
			$page_data['page_title'] = get_phrase('edit_category');
			$page_data['category_id'] = $param2;
		}
	...
	public function exam_form($param1 = "", $param2 = "")
	{
		...
		if ($param1 == "add_exam") {
			...
			$page_data['page_title'] = 'Thêm mới đề thi';
			$page_data['categories'] = $this->crud_model->get_exam_categories()->result_array();
		}
		if ($param1 == "edit_exam") {

			$page_data['page_name'] = 'exam_edit';
			$page_data['page_title'] = 'Cập nhật đề thi';
			$page_data['categories'] = $this->crud_model->get_exam_categories()->result_array();
	...
	public function banner($param1 = "", $param2 = "")
	{
		...
		if ($param1 == "add") {
			...
			$response = $this->crud_model->add_banner($post_image); // PROVIDING TRUE FOR INSTRUCTOR
			$response ? $this->session->set_flashdata('flash_message', get_phrase('banner_added_successfully')) : $this->session->set_flashdata('error_message', get_phrase('banner_code_already_exists'));
			...
			if (!$this->upload->do_upload()) {
				$errors =  array('error' => $this->upload->display_errors());
				$sbanner = $this->crud_model->get_banners($param2)->row_array();
				$post_image = $sbanner['expiry_date'];
			} else {
				$data =  array('upload_data' => $this->upload->data());
				$post_image = $data['upload_data']['file_name'];
			}

			$response = $this->crud_model->edit_banner($param2, $post_image);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('banner_updated_successfully')) : $this->session->set_flashdata('error_message', get_phrase('banner_file_already_exists'));
			redirect(site_url('admin/banner'), 'refresh');
		} elseif ($param1 == "delete") {
			// CHECK ACCESS PERMISSION
			check_permission('banner');

			$response = $this->crud_model->delete_banner($param2);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('banner_deleted_successfully')) : $this->session->set_flashdata('error_message', get_phrase('banner_code_already_exists'));
			redirect(site_url('admin/banner'), 'refresh');
		}

		$page_data['page_name'] = 'banner';
		$page_data['page_title'] = get_phrase('banner');
		$page_data['banners'] = $this->crud_model->get_banners()->result_array();
		$this->load->view('backend/index', $page_data);
	}
	...
	public function banner_form($param1 = "", $param2 = "")
	{
		...
		if ($param1 == 'add_banner_form') {

			$page_data['page_name'] = 'banner_add';
			$page_data['page_title'] = get_phrase('add_banners');
			$this->load->view('backend/index', $page_data);
		} elseif ($param1 == 'edit_banner_form') {

			$page_data['page_name'] = 'banner_edit';
			$page_data['banner'] = $this->crud_model->get_banners($param2)->row_array();
	...
	/** tieuchi functionality starts */
	public function tieuchi($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		if ($param1 == "add") {
			...
			$response = $this->crud_model->add_tieuchi($post_image); // PROVIDING TRUE FOR INSTRUCTOR
			$response ? $this->session->set_flashdata('flash_message', get_phrase('tieuchi_added_successfully')) : $this->session->set_flashdata('error_message', get_phrase('tieuchi_code_already_exists'));
			redirect(site_url('admin/tieuchi'), 'refresh');
		} elseif ($param1 == "edit") {
			// CHECK ACCESS PERMISSION
			check_permission('tieuchi');
			$config['upload_path'] = './uploads/system';
			...
			if (!$this->upload->do_upload()) {
				$errors =  array('error' => $this->upload->display_errors());
				$stieuchi = $this->crud_model->get_tieuchis($param2)->row_array();
				$post_image = $stieuchi['expiry_date'];
			} else {
				$data =  array('upload_data' => $this->upload->data());
				$post_image = $data['upload_data']['file_name'];
			}

			$response = $this->crud_model->edit_tieuchi($param2, $post_image);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('tieuchi_updated_successfully')) : $this->session->set_flashdata('error_message', get_phrase('tieuchi_file_already_exists'));
			redirect(site_url('admin/tieuchi'), 'refresh');
		} elseif ($param1 == "delete") {
			// CHECK ACCESS PERMISSION
			check_permission('tieuchi');

			$response = $this->crud_model->delete_tieuchi($param2);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('tieuchi_deleted_successfully')) : $this->session->set_flashdata('error_message', get_phrase('tieuchi_code_already_exists'));
			redirect(site_url('admin/tieuchi'), 'refresh');
		}

		$page_data['page_name'] = 'tieuchi';
		$page_data['page_title'] = get_phrase('tieuchi');
		$page_data['tieuchis'] = $this->crud_model->get_tieuchis()->result_array();
		$this->load->view('backend/index', $page_data);
	}

	public function tieuchi_form($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		if ($param1 == 'add_tieuchi_form') {

			$page_data['page_name'] = 'tieuchi_add';
			$page_data['page_title'] = get_phrase('add_tieuchis');
			$this->load->view('backend/index', $page_data);
		} elseif ($param1 == 'edit_tieuchi_form') {

			$page_data['page_name'] = 'tieuchi_edit';
			$page_data['tieuchi'] = $this->crud_model->get_tieuchis($param2)->row_array();
			$page_data['page_title'] = get_phrase('tieuchi_edit');
			$this->load->view('backend/index', $page_data);
		}
	}
	...
	/** cat_new functionality starts */
	public function cat_new($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		// CHECK ACCESS PERMISSION
		check_permission('cat_new');

		if ($param1 == "add") {
			// CHECK ACCESS PERMISSION
			check_permission('cat_new');

			$response = $this->crud_model->add_cat_new(); // PROVIDING TRUE FOR INSTRUCTOR
			$response ? $this->session->set_flashdata('flash_message', get_phrase('cat_new_added_successfully')) : $this->session->set_flashdata('error_message', get_phrase('cat_new_code_already_exists'));
			redirect(site_url('admin/cat_new'), 'refresh');
		} elseif ($param1 == "edit") {

			$response = $this->crud_model->edit_cat_new($param2);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('cat_new_updated_successfully')) : $this->session->set_flashdata('error_message', get_phrase('cat_new_file_already_exists'));
			redirect(site_url('admin/cat_new'), 'refresh');
		} elseif ($param1 == "delete") {
			// CHECK ACCESS PERMISSION
			check_permission('cat_new');

			$response = $this->crud_model->delete_cat_new($param2);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('cat_new_deleted_successfully')) : $this->session->set_flashdata('error_message', get_phrase('cat_new_code_already_exists'));
			redirect(site_url('admin/cat_new'), 'refresh');
		}

		$page_data['page_name'] = 'cat_new';
		$page_data['page_title'] = get_phrase('cat_new');
		$page_data['cat_new'] = $this->crud_model->get_cat_new()->result_array();
		$this->load->view('backend/index', $page_data);
	}

	public function cat_new_form($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		if ($param1 == 'add_cat_new_form') {
			$page_data['page_name'] = 'cat_new_add';
			$page_data['page_title'] = get_phrase('add_cat_new');
			$this->load->view('backend/index', $page_data);
		} elseif ($param1 == 'edit_cat_new_form') {

			$page_data['page_name'] = 'cat_new_edit';
			$page_data['cat_new'] = $this->crud_model->get_cat_new($param2)->row_array();
			$page_data['page_title'] = get_phrase('cat_new_edit');
			$this->load->view('backend/index', $page_data);
		}
	}
	// ADMINS SECTION ENDS
	...
	public function news($param1 = "", $param2 = "")
	{
		...
		if ($param1 == "add") {
			...
			$response = $this->crud_model->add_news($post_image); // PROVIDING TRUE FOR INSTRUCTOR
			$response ? $this->session->set_flashdata('flash_message', get_phrase('news_added_successfully')) : $this->session->set_flashdata('error_message', get_phrase('news_code_already_exists'));
			...
			if (!$this->upload->do_upload()) {
				$errors =  array('error' => $this->upload->display_errors());
				$snews = $this->crud_model->get_news($param2)->row_array();
				$post_image = $snews['thumb'];
			} else {
				$data =  array('upload_data' => $this->upload->data());
				$post_image = $data['upload_data']['file_name'];
			}

			$response = $this->crud_model->edit_news($param2, $post_image);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('news_updated_successfully')) : $this->session->set_flashdata('error_message', get_phrase('news_file_already_exists'));
			redirect(site_url('admin/news'), 'refresh');
		} elseif ($param1 == "delete") {
			// CHECK ACCESS PERMISSION
			check_permission('news');
			...
			$response ? $this->session->set_flashdata('flash_message', get_phrase('news_deleted_successfully')) : $this->session->set_flashdata('error_message', get_phrase('news_code_already_exists'));
			redirect(site_url('admin/news'), 'refresh');
		}

		$page_data['page_name'] = 'news';
		$page_data['page_title'] = get_phrase('news');
		$page_data['news'] = $this->crud_model->get_news()->result_array();
		$this->load->view('backend/index', $page_data);
	}

	public function news_form($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		$page_data['cat'] =  $this->crud_model->get_cat_new()->result_array();
		if ($param1 == 'add_news_form') {

			$page_data['page_name'] = 'news_add';
			$page_data['page_title'] = get_phrase('add_news');
			$this->load->view('backend/index', $page_data);
		} elseif ($param1 == 'edit_news_form') {

			$page_data['page_name'] = 'news_edit';
			$page_data['news'] = $this->crud_model->get_news($param2)->row_array();
			$page_data['page_title'] = get_phrase('news_edit');
			$this->load->view('backend/index', $page_data);
		}
	}
	// ADMINS SECTION ENDS
	...
	public function menu($param1 = "", $param2 = "")
	{
		...
		// CHECK ACCESS PERMISSION
		check_permission('menu');

		if ($param1 == "add") {
			// CHECK ACCESS PERMISSION
			check_permission('menu');

			$response = $this->crud_model->add_menu(); // PROVIDING TRUE FOR INSTRUCTOR
			$response ? $this->session->set_flashdata('flash_message', get_phrase('menu_added_successfully')) : $this->session->set_flashdata('error_message', get_phrase('menu_code_already_exists'));
			redirect(site_url('admin/menu'), 'refresh');
		} elseif ($param1 == "edit") {
			// CHECK ACCESS PERMISSION

			$response = $this->crud_model->edit_menu($param2);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('menu_updated_successfully')) : $this->session->set_flashdata('error_message', get_phrase('menu_file_already_exists'));
			redirect(site_url('admin/menu'), 'refresh');
		} elseif ($param1 == "delete") {
			// CHECK ACCESS PERMISSION
			check_permission('menu');

			$response = $this->crud_model->delete_menu($param2);
			$response ? $this->session->set_flashdata('flash_message', get_phrase('menu_deleted_successfully')) : $this->session->set_flashdata('error_message', get_phrase('menu_code_already_exists'));
			redirect(site_url('admin/menu'), 'refresh');
		}

		$page_data['page_name'] = 'menu';
		$page_data['page_title'] = get_phrase('menu');
		$page_data['menu'] = $this->crud_model->get_menu()->result_array();
		$this->load->view('backend/index', $page_data);
	}
	...
	public function menu_form($param1 = "", $param2 = "")
	{
		...
		check_permission('menu');
		$page_data['cat'] =  $this->crud_model->get_cat_new()->result_array();
		$page_data['menus'] = $this->crud_model->get_menu_sub('0')->result_array();
		$page_data['news'] = $this->crud_model->get_news()->result_array();
		if ($param1 == 'add_menu_form') {
			$page_data['page_name'] = 'menu_add';
			$page_data['page_title'] = get_phrase('add_menu');
			$this->load->view('backend/index', $page_data);
		} elseif ($param1 == 'edit_menu_form') {

			$page_data['page_name'] = 'menu_edit';
			$page_data['menu'] = $this->crud_model->get_menu($param2)->row_array();
			$page_data['page_title'] = get_phrase('menu_edit');
			$this->load->view('backend/index', $page_data);
		}
	}
	// ADMINS SECTION ENDS
	...
	public function upload_image()
	{
		...
		if (!$this->upload->do_upload('image')) {
			...
			$new_image_width    = $data['image_width'];
			if ($new_image_width > 500) $new_image_width = 500;
			$new_image_height   = $data['image_height'];
			// print_r($data);
			header('Content-Type: application/json');
			echo '{"data":{"id":"sudjIzE","title":null,"description":null,"datetime":1618468118,"type":"image\/png","animated":false,"width":' . $new_image_width . ',"height":' . $new_image_height . ',"size":30740,"views":0,"bandwidth":0,"vote":null,"favorite":false,"nsfw":null,"section":null,"account_url":null,"account_id":0,"is_ad":false,"in_most_viral":false,"has_sound":false,"tags":[],"ad_type":0,"ad_url":"","edited":"0","in_gallery":false,"deletehash":"2V5O9OPaz03ZR34","name":"","link":"' . base_url('/uploads/other/') . $data['file_name'] . '"},"success":true,"status":200}';
		}
	}
	...
	/** news functionality starts */
	public function dangkyhoc($param1 = "", $param2 = "")
	{
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		// CHECK ACCESS PERMISSION
		check_permission('dangkyhoc');
		echo $param1;
		if ($param1 == 'xeplop') {
			$data['status'] = 1;
			$this->db->where('id', $param2);
			$this->db->update('dangkyhoc', $data);
			redirect(site_url('admin/dangkyhoc'), 'refresh');
		}
		if ($param1 == 'huy') {
			$data['status'] = 2;
			$this->db->where('id', $param2);
		...
		$page_data['page_name'] = 'dangkyhoc';
		$page_data['page_title'] = 'Danh sách đăng ký học';
		$page_data['dangkyhoc'] = $this->crud_model->get_dangkyhoc()->result_array();
		$this->load->view('backend/index', $page_data);
	}

	public function experts($param1 = null, $param2 = null)
	{
		$this->verifyAdmin();
		switch ($param1) {
			case 'add':
				return ExpertService::add();
			case 'edit':
				return ExpertService::edit();
			case 'delete':
				return ExpertService::delete($param2);
			default:
				return ExpertService::list();
		}
	}

	public function course_expert($param1 = null, $param2 = null)
	{
		$this->verifyAdmin();
		switch ($param1) {
			case 'add':
				return CourseExpertService::add();
			case 'edit':
				return CourseExpertService::edit();
			case 'delete':
				return CourseExpertService::delete($param2);
			default:
				return CourseExpertService::list();
		}
	}
