...
class Home extends CI_Controller
{
	...
	public function de_thi($id = ""){
		if($id == ""){
			...
			$page_data['page_title'] = 'Đề thi';
			$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);

		}else{
			
			$exam_details = $this->crud_model->get_lessons('lesson', $id)->row_array();

			if($exam_details && $exam_details['lesson_type'] == 'exam'){
				$page_data['page_name'] = "exam_show";
				$page_data['exam_details'] = $exam_details;
				$page_data['page_title'] = $exam_details['title'];
				$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);

			}else{

				redirect(site_url('de-thi'));
			}
	...
	public function thongtin_chuyenkhoan()
	{
		$page_data['total_price_of_checking_out'] = $this->session->userdata('total_price_of_checking_out');
		$page_data['page_title'] = site_phrase("payment_gateway");
		$this->load->view('payment/thongtin_chuyenkhoan', $page_data);
	}
	public function chuyenkhoan_checkout($payment_request = "only_for_mobile")
	{
		// $this->crud_model->enrol_student($user_id);
		$this->crud_model->course_purchase($this->session->userdata('user_id'), 'chuyenkhoan', $this->session->userdata('total_price_of_checking_out'));
		$this->email_model->course_purchase_notification($user_id, 'chuyenkhoan', $this->session->userdata('total_price_of_checking_out'));
		...
		// endif;
		redirect('home/thongtin_chuyenkhoan', 'refresh');
	}
	...
	{
		dd('here');
		$page_data['page_name'] = 'bai_viet';
		$page_data['news'] = $this->crud_model->get_news($id)->row_array();
		$page_data['page_title'] = get_phrase('news_edit');
		$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
	}
	//FOR MOBILE
	public function dang_muc($id = "")
	{
		$page_data['page_name'] = 'dang_muc';
		$page_data['list_news'] = $this->crud_model->get_news_cat($id)->result_array();
		$page_data['cat_new'] = $this->crud_model->get_cat_new($id)->row_array();
		$page_data['page_title'] = $page_data['cat_new']['name'];
		$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
	}
	public function dang_ky_ngay()
	{
		if (isset($_POST['hoten']) && !empty($_POST['hoten'])) {
			$this->crud_model->dang_ky_ngay();
		}
		$page_data['page_name'] = 'finish';
		$page_data['page_title'] = 'Đăng ký học hoàn thành';
		$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
	}
	public function giang_vien()
	{
		$page_data['page_name'] = 'giang_vien';
		$page_data['page_title'] = 'Danh sách giảng viên';
		$page_data['instructors'] = $this->user_model->get_instructor()->result_array();
		$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
	}

	public function get_course_for_you()
	{
		$category_id = $_GET['cat_id'];
		$categories = $this->db->select('id')->get_where('category', [
			'parent' => $category_id
		], 4)->result_array();
		$aryIdCate = array_column($categories, 'id');
		$course = $this->db->where_in('sub_category_id', $aryIdCate)->get('course')->result_array();
		$this->load->view('frontend/' . get_frontend_settings('theme') . '/components/course_box_wrap', [
			'courses' => $course
		]);
	}
}
