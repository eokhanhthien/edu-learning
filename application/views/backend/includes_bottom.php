...
<script src="<?php echo base_url('assets/backend/js/nicEdit.js?_'.time()); ?>"></script>
<!-- dragula js-->
...
<?php if (isset($add_script) && $add_script === true): ?>
  <script src="<?= site_url("assets/backend/js/$page_name.js") ?>"></script>
<?php endif; ?>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    bkLib.onDomLoaded(function() {
        new nicEditor({iconsPath : '<?php echo site_url('assets/backend/js/nicEditorIcons.gif');?>',fullPanel : true}).panelInstance('area1');
  });
...
function error_required_field() {
  $.NotificationApp.send("<?php echo get_phrase('oh_snap'); ?>!", "<?php echo get_phrase('please_fill_all_the_required_fields'); ?>" ,"top-right","rgba(0,0,0,0.2)","error");
	...
	$(document).on('change', '#get_url', function() {
  $('#name').val($(this).find(':selected').data('name'));
  $('#url').val($(this).find(':selected').data('url'));
});
</script>
