					...
					foreach ($menu_root as $key => $menu){ 
						...
						if($sub_menu){
							...
							foreach ($sub_menu as $key => $menus){
								$sub_menu3= $this->crud_model->get_menu_sub($menus['id'])->result_array();
								if($sub_menu3){
									echo '<li><span><a href="javascript::" title="'.$menus['name'].'" alt="'.$menus['name'].'">'.$menu['name'].'<i class="icon-down-dir"></i></a></span><ul>';
									foreach ($sub_menu3 as $keys => $menuss){
										echo '<li><span><a href="'.$menuss['url'].'" title="'.$menuss['name'].'" alt="'.$menus['name'].'">'.$menuss['name'].'</a></span></li>';
									}
									echo '</ul></li>';
									}else{
