...
$reviewVideoUrl = $this->crud_model->getOneVideoDemoCourse($course_details);
?>
      ...
      <div class="col-12">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-5">
            <?php $categoryForeginOfCourse = $this->crud_model->getCategoryForeginOfCourse($course_details['id']); ?>
            <li class="breadcrumb-item"><a href="/">Home</a></li>
            <li class="breadcrumb-item">
              <a href="#<?= $categoryForeginOfCourse['level1']['slug'] ?>"><?= $categoryForeginOfCourse['level1']['name'] ?></a>
            </li>
            <li class="breadcrumb-item active" aria-current="page">
              <a href="#<?= $categoryForeginOfCourse['level2']['slug'] ?>"><?= $categoryForeginOfCourse['level2']['name'] ?></a>
            </li>
            <li class="breadcrumb-item active" aria-current="page"><?= $course_details['title']; ?></li>
          </ol>
        </nav>
      </div>
    </div>
    <div class="row align-items-end">
      <div class="col-lg-8">
        <div class="course-header-wrap">
          <h2 class="title mb-4"><?php echo $course_details['title']; ?></h2>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-8 mt-5">
        <h2 class="mb-3"><?= site_phrase('what_will_i_learn') ?></h2>
        <div class="what-you-get-box mb-5">
          <ul class="what-you-get__items">
            <?php foreach (json_decode($course_details['outcomes']) as $outcome) : ?>
              <?php if ($outcome != "") : ?>
                <li class="mb-0"><?= $outcome; ?></li>
              <?php endif; ?>
            <?php endforeach; ?>
          </ul>
        </div>
        <?php if ($course_details['course_type'] == 'general') : ?>
          <div class="course-curriculum-box">
            <div class="course-curriculum-title mt-4">
              <h2 class="mb-3"><?= site_phrase('curriculum_for_this_course'); ?></h2>
              <div class="mb-3">
                <span class="total-lectures">
                  <?= $this->crud_model->get_lessons('course', $course_details['id'])->num_rows() . ' ' . site_phrase('lessons'); ?>
                </span>
                <span class="dots ml-1 mr-1">.</span>
                <span class="total-time">
                  <?= $this->crud_model->get_total_duration_of_lesson_by_course_id($course_details['id']) ?>
                </span>
              </div>
            </div>
                    ...
                    <ul>
                      ...
                      foreach ($lessons as $lesson) : ?>
                        <li class="lecture has-preview">
                          <span class="lecture-title" onclick="go_course_playing_page('<?php echo $course_details['id']; ?>', '<?php echo $lesson['id']; ?>')"><?php echo $lesson['title']; ?></span>
        ...
        <div class="requirements-box">
          <h2 class="mb-3"><?= site_phrase('requirements') ?></h2>
          <div class="what-you-get-box mb-5">
            <ul class="what-you-get__items">
              <?php foreach (json_decode($course_details['requirements']) as $requirement) : ?>
                <?php if ($requirement != "") : ?>
                  <li class="mb-0"><?php echo $requirement; ?></li>
                <?php endif; ?>
              <?php endforeach; ?>
            </ul>
          </div>
        ...
        <div class="about-instructor-box">
          <div class="about-instructor-title">
            <?= site_phrase('about_the_instructor'); ?>:
          </div>
          <?php if ($course_details['multi_instructor']) : ?>
            <?php $instructors = $this->user_model->get_multi_instructor_details_with_csv($course_details['user_id']); ?>
            <?php foreach ($instructors as $key => $instructor) : ?>
          ...
          <?php else : ?>
            <div class="row">
              <div class="col-12">
                <a href="<?= site_url('home/instructor_page/' . $instructor_details['id']); ?>">
                  <?= $instructor_details['first_name'] . ' ' . $instructor_details['last_name']; ?>
                  <img alt="Giảng viên" data-src="<?= base_url('uploads/icons/icon-profile-police.svg') ?>" width="16px" class="lazyload" src="<?= base_url('uploads/icons/icon-profile-police.svg') ?>">
                </a>
                <div class="text-small" style="color:#90a0b7; font-size: 14px;">
                  <?= $instructor_details['title'] ?>
                </div>
              </div>
              <div class="col-12 d-flex mt-3 align-items-center">
                <div class="about-instructor-image">
                  <img src="<?= $this->user_model->get_user_image_url($instructor_details['id']); ?>" alt="" class="img-fluid">
                </div>
                <ul style="list-style: none;" class="mb-0">
                  <li class="mb-3">
                    <i class="fas fa-comment mr-3"></i>
                    <b><?= $this->crud_model->get_instructor_wise_course_ratings($instructor_details['id'], 'course')->num_rows() ?></b>
                    <?= site_phrase('reviews'); ?>
                  </li>
                  <li class="mb-3">
                    <i class="fas fa-user mr-3"></i>
                    <b><?= $this->crud_model->get_enrol_of_teacher($instructor_details['id']) ?></b>
                    <?= site_phrase('students') ?>
                  </li>
                  <li class="mb-3">
                    <i class="fas fa-play-circle mr-3"></i>
                    <b><?= $this->crud_model->get_instructor_wise_courses($instructor_details['id'])->num_rows(); ?></b>
                    <?= site_phrase('course'); ?></li>
                </ul>
              </div>
              <div class="col-12 mt-4">
                <div class="instructor-bio">
                  <?= $instructor_details['biography'] ?>
                </div>
              </div>
            </div>
          <?php endif; ?>
        ...
        <div class="compare-box view-more-parent">
          ...
          <div class="compare-courses-wrap">
            <?php 
              $other_realted_courses = $this->crud_model->get_realted_courses(
                $course_details['id'],
                $course_details['category_id'],
                $course_details['sub_category_id']
              );
            ?>
          ...
          <?php if (!empty($other_realted_courses)): ?>
            <div class="view-more" onclick="viewMore(this)">+ <?php echo site_phrase('view_more'); ?></div>
          <?php else : ?>
            <div class="alert alert-warning">Không có khóa học liên quan</div>
          <?php endif; ?>
        </div>
        ...
        <div class="course-sidebar natural">
          <?php if ($reviewVideoUrl != "") : ?>
            <div class="preview-video-box">
              <a data-toggle="modal" data-target="#CoursePreviewModal">
                <img src="<?= $this->crud_model->get_course_thumbnail_url($course_details['id']); ?>" alt="" class="img-fluid">
                <span class="preview-text"><?= site_phrase('preview_this_course'); ?></span>
          ...
          <div class="course-sidebar-text-box">
            ...
            <?php if (is_purchased($course_details['id'])) : ?>
              ...
              <div class="already_purchased">
                <a href="<?php echo site_url('home/lesson/' . slugify($course_details['title']) . '/' . $course_details['id']); ?>">Vào học</a>
              </div>
            <?php else : ?>
              <div class="box-voucher voucher-item my-3 border-0" style="border-radius: 6px;">
                <div class="d-lg-flex justify-content-between">
                    <div class="mt-0 d-flex justify-content-between w-100 bg-white" style="border: 1px solid #90A0B7;border-radius: 6px">
                        <div class="my-auto mx-2">
                            <i class="fa fa-percent" style="color: #727272"></i>
                        </div>
                        ...
                        <button type="button" class="fw-600 btn btn-gitiho-default ml-2 round-4 button-apply-coupon text-nowrap my-2" data-course="" style="outline: none; box-shadow: none; border-left: 1px solid rgb(144, 160, 183); padding: 0px 0.7rem; background: white; color: rgb(33, 150, 83); font-size: 14px; border-radius: 0px 4px 4px 0px !important;" onclick="applyCoupon(this)">Áp dụng</button>
                    </div>
                </div>
              </div>
              <!-- WISHLIST BUTTON -->
              ...
              <ul>
                <?php if ($course_details['course_type'] == 'general') : ?>
                  <li>
                    ...
                    <?= $this->crud_model->get_total_duration_of_lesson_by_course_id($course_details['id']) . ' ' . site_phrase('on_demand_videos') ?>
                  </li>
                  <li>
                  ...
                  <li>
                    ...
                    <?= site_phrase('access_on_mobile_and_tv'); ?>
                  </li>
                <?php elseif ($course_details['course_type'] == 'scorm') : ?>
                  ...
                  <li>
                    <i class="fas fa-mobile-alt"></i>
                    <?= site_phrase('access_on_laptop_and_tv'); ?>
                  </li>
                <?php endif; ?>
                <li>
                  <i class="far fa-compass"></i><?php echo site_phrase('full_lifetime_access'); ?>
                </li>
                <li>
            ...
            <div class="register-to-company">
              <div class="p-3" style="border-top: 1px solid rgba(193, 209, 231, 0.47)">
                <p class="fw-700 color_label mb-1">Đăng ký cho doanh nghiệp</p>
                <p class="mb-3 color_label">Giúp nhân viên của bạn truy cập không giới hạn 450 khoá học, mọi lúc, mọi nơi</p>
                <a href="#" target="_blank" rel="noreferrer nofollow noopener">
                  <button class="btn btn-custom-white w-100 round-4" style="border: 1px solid #333333">Đăng ký nhận tư vấn</button>
                </a>
              </div>
            </div>
          </div>
...
<?php if ($reviewVideoUrl != "") :
  ...
  if ($course_details['course_overview_provider'] == "html5") {
    ...
    $video_details = $this->video_model->getVideoDetails($reviewVideoUrl);
            ...
            <div class="embed-responsive embed-responsive-16by9">
              <?php if (strtolower(strtolower($provider)) == 'youtube') : ?>
                ...
                <div class="plyr__video-embed" id="player">
                  <iframe height="500" src="<?= $reviewVideoUrl; ?>?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" allowfullscreen allowtransparency allow="autoplay"></iframe>
                </div>
              ...
              <?php else : ?>
                <!------------- PLYR.IO ------------>
                <link rel="stylesheet" href="<?php echo base_url(); ?>assets/global/plyr/plyr.css">
                <video poster="<?= $this->crud_model->get_course_thumbnail_url($course_details['id']); ?>" id="player" playsinline controls>
                  <?php if (get_video_extension($reviewVideoUrl) == 'mp4') : ?>
                    <source src="<?= $reviewVideoUrl; ?>" type="video/mp4">
                  <?php elseif (get_video_extension($reviewVideoUrl) == 'webm') : ?>
                    <source src="<?= $reviewVideoUrl; ?>" type="video/webm">
                  <?php else : ?>
