<?php
$quiz_questions = $this->crud_model->get_quiz_questions($exam_details['id']);
?>
<div class="mt-5" style="text-align: center;">
    <div id="quiz-body">
        <div class="" id="quiz-header" style="border: 1px solid #eee;padding: 10px;box-shadow: 2px 3px 4px 3px grey;">
            <i class="fa fa-book"></i> Trắc nghiệm : <strong><?php echo $exam_details['title']; ?></strong><br>
            ...
            <i class="fas fa-clock"></i> <?php echo get_phrase("quiz_time"); ?> : <strong><?php echo $exam_details['duration']; ?> (phút)</strong><br>
        ...
        <form id = "quiz_form" action="" method="post" onsubmit="return false;">
            <h3 id="title-question" class="hidden" style="text-align: center;"><?php echo $exam_details['title']; ?></h3>
            <div id="timer-panel" class="hidden">
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-8">
                        <div class="w-full rounded-full bg-gray-300" style="height: 9px;">
                            <div class="w-1/2 h-full bg-indigo-600 rounded-full process-bar" style="width: 0%;"></div>
                        </div>
                    </div>
                </div>
            ...
            <?php if (count($quiz_questions->result_array()) > 0): ?>
                ...
                ?>
                    <input type="hidden" name="lesson_id" value="<?php echo $exam_details['id']; ?>">
                    <div class="question-content hidden" id = "question-number-<?php echo $key+1; ?>">
                        <div class="row justify-content-center">
                            <div class="col-lg-8">
                                <div class="card text-left quiz-card">
                                    <div class="card-body">
                                    ...
                                    <ul class="list-group list-group-flush">
                                        ...
                                        <li class="list-group-item quiz-options">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="<?php echo $quiz_question['id']; ?>[]" value="<?php echo $key2+1; ?>" id="quiz-id-<?php echo $quiz_question['id']; ?>-option-id-<?php echo $key2+1; ?>" onclick="enableQuestionButton('<?php echo $key+1;?>')">
    ...
    <div class="panel-question hidden">
        <div id="list-questions">
            <div class="col-lg-8">
                <div class="row justify-content-center" style="border: 1px solid #eee;background: black;border-radius: 10px;">
                    <?php if (count($quiz_questions->result_array()) > 0): ?>
                        <?php foreach ($quiz_questions->result_array() as $key => $quiz_question):
                                $options = json_decode($quiz_question['options']);
                        ?>
                        <div class="item btn-question-<?=$key+1; ?>" onclick="showQuestion(<?=$key+1; ?>)"><?php echo $key+1;?></div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            </div>
...
<style>
    ...
    #timer-panel .rounded-full {
        border-radius: 9999px;
    }

    #timer-panel .bg-gray-300 {
        --tw-bg-opacity: 1;
...
function getStarted(first_quiz_question) {
    ...
    startCountdown(<?=$exam_details['duration']; ?>);
}
...
function submitQuiz() {
    let confirm = window.confirm('Bạn đã trả lời '+ questionAnswered +'/'+ totalQuestion + ' câu hỏi. Bạn muốn xác nhận nộp bài bài trắc nghiệm ?');
...
var totalQuestion = <?php echo count($quiz_questions->result_array()); ?>;

function showQuestion(ID){
    $('.question-content').hide();
    $('#question-number-'+ID).show();

}
function enableNextButton(quizID) {
    $('#next-btn-'+quizID).prop('disabled', false);
}
function enableQuestionButton(ID) {
    if( !$('.btn-question-'+ID).hasClass('active')){
        $('.btn-question-'+ID).addClass('active');
        questionAnswered ++;
        let percent = (questionAnswered/totalQuestion)*100;
